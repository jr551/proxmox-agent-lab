...
- `docs/commands.md`: a generated map of every `proxmox-lab` subcommand,
  regenerated by `scripts/gen-commands.py` and kept in sync by a CI
  `--check` step.

### Changed

- Rewrote `README.md` with a clearer title, tighter structure, and a link to
  the generated command reference.

### Fixed

- Documented that the `onboard build-iso` auto-install ISO must be booted in
  **UEFI** mode: under legacy BIOS the installer partitions and populates the
  disk but leaves the bootloader uninstalled (the `grub-install i386-pc`
  failure is swallowed by a non-fatal warning), producing an unbootable host.
  Verified end-to-end under OVMF: install, boot to login, and first-boot
  pairing callback with `api_verified`.
