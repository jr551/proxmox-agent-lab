...
> **New reader?** The [documentation index](docs/README.md) lists the shortest path for every task.
...
records an audit trail. Work happens inside a **lease**; ordinary leases clean
up and verify shutdown when they end. Long-term leases deliberately keep the
host on for persistent machines.

## What you need

- A spare PC running [Proxmox VE 8 or 9](https://www.proxmox.com), with a wired
  NIC and Wake-on-LAN, or an alternative power mode.
- Python 3.11+ on the controller.
- A place to store secrets. The default `auto` backend reads
  `PROXMOX_AGENT_LAB_*` environment variables; explicit options are
  `keychain`, `secret-tool` and `file`. See the [secrets backend
  guide](docs/CONFIGURATION.md#secrets).

## Install

One touch — installs, configures, stores secrets, and health-checks:

```bash
curl -fsSL https://raw.githubusercontent.com/jr551/proxmox-agent-lab/main/install.sh | bash
```

No install at all? `bootstrap.sh` drops a throwaway environment under
...
Full setup, including Proxmox host preparation and the audit ledger, is in
[docs/INSTALL.md](docs/INSTALL.md). Configuration details are in
[docs/CONFIGURATION.md](docs/CONFIGURATION.md).

## First safe workflow

Every task follows this shape. `lease-end` is in a `trap` so it runs even if the
work fails:
...
lease skeleton is in [SKILL.md](SKILL.md); troubleshooting is in
...
For an ordinary lease, the host wakes, a guest is cloned, work runs, and
`lease-end` destroys the lease's guests and verifies the host is off. For
long-term work, a `lease-begin --long-term` keeps the host and its guests
running until you run `lease-destroy --confirm`. Read the details in
...
- **Verified shutdown.** `lease-end` does not claim success until the API stops
  answering, twice.
...
- **Secrets stay in the configured backend.** They never appear in `argv`, the
...
- [docs/AUDIT-2026-08-24.md](docs/AUDIT-2026-08-24.md) — security audit report
